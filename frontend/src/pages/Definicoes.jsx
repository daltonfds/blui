  ...
  return (
    <LayoutApp>
      ...
      <p className="text-sm text-base-ink/55 mb-8">
        Liga as tuas contas de anúncios para ativar campanhas e tracking.
      </p>

      <form
        onSubmit={ligarConta}
        className="bg-base-white border border-black/5 rounded-xs p-6 max-w-lg space-y-4"
      >
        <p className="text-xs text-base-ink/45">
          Obtém estes valores no Gestor de Anúncios do Meta
          (Business Settings → Utilizadores do sistema) após autorizares
          a aplicação Blui.
        </p>
        ...
        <Campo label="ID da conta de anúncios">
          <input
            ...
            onChange={(e) =>
              setForm({ ...form, ad_account_id: e.target.value })
            }
          />
        </Campo>
        ...
        <Campo label="WhatsApp Phone Number ID">
          <input
...
function Campo({ label, children }) {
  const filho = React.cloneElement(children, {
    className:
      'w-full border border-black/10 rounded-xs px-3.5 py-2.5 text-sm text-base-ink focus:border-brand-500 transition-colors',
  });
